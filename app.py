...
def venues():
  ...
  for venue in Venue.query.all():
    foundPlace = False
    for obj in data:
      if venue.city == obj["city"] and venue.state == obj["state"]:
        obj["venues"].append({
          "id": venue.id,
          "name": venue.name,
...
def search_venues():
  ...
  ans = Venue.query.filter(Venue.name.ilike(f"%{request.form.get('search_term', '')}%")).all()
...
def create_venue_submission():
  ...
  try:
    ...
    error = False
    errors = []
    if not re.match(r"^([\s\d]+)$",request.form.get("phone")):
      error = True
      errors.append("invalid phone number")
    if not re.match(r"(?:(?:http|https):\/\/)?(?:www.)?facebook.com\/(?:(?:\w)*#!\/)?(?:pages\/)?(?:[?\w\-]*\/)?(?:profile.php\?id=(?=\d.*))?([\w\-]*)?",request.form.get("facebook_link")):
      error = True
      errors.append("invalid facebook link")
    
    if error:
      flash(" and ".join(errors))
...
def show_artist(artist_id):
  ...
  #}
  #dataa = list(filter(lambda d: d['id'] == artist_id, [data1, data2, data3]))[0]
...
def edit_artist(artist_id):
  form = ArtistForm()
  ast = Artist.query.get(artist_id)
  artist={
    "id": artist_id,
    "name": ast.name,
    "genres": ast.genres.split(","),
    "city": ast.city,
    "state": ast.state,
    "phone": ast.phone,
    "website": ast.phone,
    "facebook_link": ast.facebook_link,
    "seeking_venue": True,
    "seeking_description": "Looking for shows to perform at in the San Francisco Bay Area!",
    "image_link": "https://images.unsplash.com/photo-1549213783-8284d0336c4f?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=300&q=80"
  }
...
def edit_artist_submission(artist_id):
  ...
  if error:
    ...
    ast = Artist.query.get(artist_id)
    ast.name=request.form.get("name")
    ast.city=request.form.get("city"),
    ast.state=request.form.get("state"),
    ast.phone=request.form.get("phone"),
    ast.genres=",".join(request.form.getlist("genres")),
    ast.facebook_link=request.form.get("facebook_link")
...
def edit_venue_submission(venue_id):
  ...
  if error:
    ...
    ven.genres=",".join(request.form.getlist("genres")),
